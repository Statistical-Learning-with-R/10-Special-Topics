...
stick with the simple one for now.)
...
The model specification function for a very simple Neural Network is `MLPClassifier`, which 
stands for "multilayer perceptron classifier".  For now, we will use only one hidden layer. 

The parameters we will focus on are:

* `hidden_layer_sizes`: An array giving the number of nodes in each hidden layer.  Since
we are using only one layer, this is just one number.
* `max_iter`:  The number of updates ("epochs") to run
* `activation`: The activation function on the hidden layer. Possible values are: ‘identity’, ‘logistic’, ‘tanh’, ‘relu’
